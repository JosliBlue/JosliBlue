...
export const CodeBlock: React.FC<CodeBlockProps> = ({ code, skillIcons = {} }) => {
                ...
                if (/^"/.test(match)) {
                    if (/:$/.test(match)) {
                        ...
                        if (skillIcons[content]) {
                            return `<span class="${cls} skill-item hover:text-accent cursor-help transition-colors" data-skill="${content}">${match}</span>`;
                        }
                    }
    ...
    return (
        <div className="rounded-md overflow-hidden border border-[var(--code-border)] bg-[var(--bg-code)] my-6 shadow-sm group relative">
            ...
            <div className="p-4" onMouseMove={handleMouseMove} onMouseLeave={() => setHoveredSkill(null)}>
            ...
            {hoveredSkill && skillIcons[hoveredSkill.name] && (
                ...
                >
                    <Icon icon={skillIcons[hoveredSkill.name]} width="50" height="50" className="text-accent" />
                </div>
            )}
        </div>
    );
